...
export const groqProvider = {
  ...
  async generate(request) {
    ...
    try {
      if (!auth.currentUser) {
        throw new AIError(AIErrorTypes.AI_AUTH_ERROR, 'User must be authenticated to use AI features.', 'groq');
      }
      ...
      console.error(`EXACT EXCEPTION in Render backend (groqProvider.js):`, err.message);
      analyticsService.trackError('Groq API Error (Render)', err);
      ...
      throw new AIError(errorType, err.message || 'Unknown error occurred.', 'groq');
    }

    const responseTime = Date.now() - startTime;
    console.log(`[Groq API] Cloud Function Request completed. Time: ${responseTime}ms`);
