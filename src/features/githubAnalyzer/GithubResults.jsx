...
const InsightItem = ({ icon: Icon, color, title, desc }) => (
  <li className="flex items-start gap-3">
    <Icon size={18} className={`${color} shrink-0 mt-0.5`} strokeWidth={2.5} />
    <div>
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-100 pb-5">
        ...
        <button
          onClick={onReset}
          className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-100 text-slate-600 text-sm font-bold rounded-xl hover:bg-slate-50 transition-colors shadow-sm"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 flex flex-col sm:flex-row gap-6 items-start">
        ...
        <div className="bg-white rounded-2xl py-6 px-6 md:px-8 shadow-sm border border-slate-100 flex flex-col sm:flex-row gap-8 items-center h-fit">
           <div className="flex flex-col items-center shrink-0">
        ...
        <div className="divide-y divide-slate-100">
          {repos.length === 0 ? (
            <div className="p-8 text-center flex flex-col items-center">
              <p className="text-slate-600 text-[14px] font-bold mb-1">No public repositories found</p>
              <p className="text-slate-400 text-[13px] font-medium max-w-sm">
                Ensure your GitHub repositories are set to public so they can be analyzed.
              </p>
            </div>
          ) : (
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
          ...
          <ul className="space-y-4">
            {strengths.map((item, idx) => {
              ...
              return <InsightItem key={idx} icon={CheckCircle} color="text-[#10B981]" title={title} desc={desc} />;
            })}
          </ul>
        </div>
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
          ...
          <ul className="space-y-4">
            {weaknesses.map((item, idx) => {
              const { title, desc } = parseInsight(item);
              return <InsightItem key={idx} icon={AlertTriangle} color="text-[#F59E0B]" title={title} desc={desc} />;
            })}
          </ul>
        </div>
      </div>
